  ...
  def self.create_heritages
    ...
    ActiveRecord::Base.transaction do
      data.each do |item|
        ...
        heritage.mobile_image.attach(io: File.open(mobile_img_path), filename: 'top_mobile.png',
                                     content_type: 'image/png')
        4.times do
          heritage.slideshow_images.attach(io: File.open(sub_img_path), filename: 'sub.png',
                                           content_type: 'image/png')
        end

        tag_ids.each do |tag_id|
          HeritageTag.create!(heritage_id: heritage.id, tag_id:)
        end
        ...
        heritage_charms_data.each do |charm_data|
          ...
          if charm_data['order'] == 1 || charm_data['order'] == 2
            heritage_charm.main_image.attach(io: File.open(main_img_path), filename: 'main.png',
                                             content_type: 'image/png')
            heritage_charm.sub_image.attach(io: File.open(sub_img_path), filename: 'sub.png',
                                            content_type: 'image/png')
          else
            heritage_charm.main_image.attach(io: File.open(sub_img_path), filename: 'sub.png',
                                             content_type: 'image/png')
          end
          heritage_charm.save!
        end
